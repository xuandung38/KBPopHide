...
@MainActor
final class USBMonitor: ObservableObject, USBMonitorProtocol, @unchecked Sendable {
    ...
    nonisolated func processDevice(_ device: io_object_t) {
        ...
        if isKeyboard && vendorID > 0 && productID > 0 {
            let capturedVendorID = vendorID
            let capturedProductID = productID
            Task { @MainActor [weak self] in
                self?.onKeyboardConnected?(capturedVendorID, capturedProductID)
            }
        }
    }
